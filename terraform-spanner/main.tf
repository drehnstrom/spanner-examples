...
resource "google_spanner_database" "test-database" {
  ...
  ddl = [
    ...
    "CREATE TABLE Pets (PetID STRING(36) NOT NULL, OwnerID STRING(MAX) NOT NULL, PetType STRING(MAX) NOT NULL, PetName STRING(MAX) NOT NULL, Breed STRING(MAX) NOT NULL) PRIMARY KEY (PetID)",
  ]
}
